...
exports.sign_up=async(req,res)=>{
    ...
    try {
        ...
        if(existingUser){
            return res.status(400).json({message:"user alredy existed",success:false})
        }
        ...
        await result.save()
        const token=jwt.sign({username:result.username,role:result.role,id:result._id},SCRETE_KEY)
