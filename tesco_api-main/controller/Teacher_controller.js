...
exports.get_one_teacher = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: "Data fetch succesfully",
      success: true,
      data: { username, firstname, lastname, middlename },
    });
  } catch (err) {
    res.status(500).send(err);
  }
...
exports.count_all_teachers = async (req, res) => {
  try {
    const count = await AdminModal.countDocuments({ role: "teacher" });

    console.log(count);
    res.status(200).json({
      message: "Teachers count fetch successfully",
      success: true,
      count,
    });
  } catch (err) {
...
// exports.count_all_teachers = async (req, res) => {
//   try {
//     const count = await AdminModal.find();
